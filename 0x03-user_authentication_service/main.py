...
def profile_logged(session_id: str) -> None:
    ...
    """
    url = "http://your-web-server-base-url"
    data = requests.get(url + "/profile", cookies={"session_id": session_id})
...
def log_out(session_id: str) -> None:
    ...
    """
    url = "http://your-web-server-base-url"
    dat = requests.delete(url + "/sessions",
                          cookies={"session_id": session_id})
    assert dat.status_code == 200
    assert dat.json() == {}
    session_id = dat.cookies.get("session_id")
...
def update_password(email: str, reset_token: str, new_password: str) -> None:
    ...
    """
    url = "http://your-web-server-base-url"
    dat = {"email": email, "reset_token": reset_token,
           "new_password": new_password}
    data = requests.put(url + "/reset_password", dat=dat)
    assert data.status_code == 200
